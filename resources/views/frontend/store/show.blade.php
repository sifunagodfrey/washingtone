    ...
    <div class="container">
        <div class="row g-5 align-items-start">
            <div class="col-lg-4 text-center" data-aos="fade-right">
                @if($product->image)
                <img src="{{ asset("uploads/store/" . $product->image) }}" class="img-fluid rounded shadow" style="max-height:400px;object-fit:cover;" alt="{{ $product->title }}">
                @else
            ...
            <div class="col-lg-8" data-aos="fade-left">
                ...
                @php $settings = \App\Models\Setting::first(); $wa = preg_replace("/[^0-9]/", "", $settings->whatsapp_order_number ?? $settings->whatsapp_number ?? "254700000000"); @endphp
                    ...
                    <div class="col-md-6">
                        <div class="card border-success border-2 h-100">
                            <div class="card-body text-center p-4">
                                ...
                                <a href="https://wa.me/{{ $wa }}?text=Hi%20Washingtone%2C%20I%20want%20to%20order%20{{ urlencode($product->title) }}" target="_blank" class="btn btn-success rounded-pill px-4"><i class="fab fa-whatsapp me-2"></i>Order on WhatsApp</a>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="card border-primary border-2 h-100">
                            <div class="card-body text-center p-4">
                                ...
                                <p class="small text-muted mb-1">Paybill: <strong>{{ $settings->mpesa_paybill }}</strong></p>
                                @endif
    ...
    <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <form action="{{ route("store.order", $product->slug) }}" method="POST">@csrf
                ...
                @if($settings->mpesa_paybill)<small class="text-muted">Pay to Paybill <strong>{{ $settings->mpesa_paybill }}</strong></small>@endif</div>
